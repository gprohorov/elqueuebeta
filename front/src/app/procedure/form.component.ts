...
export class ProcedureFormComponent implements OnInit, OnDestroy {
    ...
    loadProcedures() {
        this.subProcedures = this.service.getAll().subscribe(
            data => {
                this.loading = false;
                this.closeAfter         = data.map(x => ({ name: x.name, value: x.id + 'pc', checked: false }) );
                this.activateAfter      = data.map(x => ({ name: x.name, value: x.id + 'pa', checked: false }) );
                this.mustBeDoneBefore   = data.map(x => ({ name: x.name, value: x.id + 'pm', checked: false }) );
    ...
    initProcedures() {
        this.model.card.closeAfter.forEach(id => {
            const p = this.closeAfter.find(x => x.value === id + 'pc');
            if (p) p.checked = true;
        });
        this.model.card.activateAfter.forEach(id => {
            const p = this.activateAfter.find(x => x.value === id + 'pa');
            if (p) p.checked = true;
        });
        this.model.card.mustBeDoneBefore.forEach(id => {
            const p = this.mustBeDoneBefore.find(x => x.value === id + 'pm');
            if (p) p.checked = true;
        });
    }
