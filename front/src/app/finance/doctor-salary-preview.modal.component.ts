...
@Component({
    templateUrl: './doctor-salary-preview.modal.component.html'
})
export class DoctorSalaryPreviewModalComponent implements IModalDialog {
    ...
    editDoctor() {
        window.open('/#/doctor-form;id=' + this.doctor, '_blank');
    }
    ...
    load() {
        ...
        this.loading = true;
        this.data = [];
        this.sub = this.financeService.getDoctorSalaryPreview(this.doctor, this.from, this.to)
            .subscribe(data => {
                this.data = data;
                this.loading = false;
            },
